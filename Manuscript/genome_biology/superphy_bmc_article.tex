...
Uploaded genomes undergo the following checks to ensure the data are of a minimum quality, and that the genomes being uploaded belong to the species \textit{E. coli}: 1) The presence of at least 1500 `conserved core' pan-genome regions; and 2) The presence of at least three \textit{E. coli} species-specific regions. The derivation of these markers is presented in the `Pan-genome' subsection of the `Example analyses', and their distribution among 2324 genomes presented in Figure \ref{fig:specific_scatter}. As can be seen, not all species-specific markers were found to be present in all strains; however, most \textit{E. coli} genomes contained at least 8 of the markers and all contained at least 3 given the quality checks described above. A general trend of genomes with higher ratios of `Genome size' / No. contigs containing both more `conserved core' regions and species-specific regions was observed.
...
Additionally, we endeavored to identify genomic regions that were specific to the species \textit{E. coli}. To achieve this we screened the `conserved core' against a subset of \textit{E. coli} and other bacterial species, the results of which are presented in Table \ref{tab:specific_distribution}. While \textit{E. coli} genomes contained more of the `conserved core' regions than any other of the genomes examined, we filtered the `conserved core' regions based on the distribution among these 12 genomes to identify genomic regions present in only the \textit{E. coli} genomes, and identified 33 candidates; the raw data table is available at \url{https://github.com/superphy/version-1/Sequences/genome_content_panseq/binary_table.txt}. These 33 candidates were screened against the GenBank `nr' and `WGS' databases using the `bacteria' taxid to limit the search; the raw Blast results are available at \url{https://github.com/superphy/version-1/Sequences/genome_content_panseq/UB0HWGTR015-Alignment.xml} and \url{https://github.com/superphy/version-1/Sequences/genome_content_panseq/UD4GVA26015-Alignment.xml}. Based on these queries using a 90\% total sequence identity threshold, we removed all genomic regions that were present in species other than \textit{E. coli}, and were left with the ten species-specific regions presented in Table \ref{tab:specific_location_function}.

Of these 2324 genomes, only 1641 had metadata beyond the name of the strain. As such, the initial SuperPhy database contained only these 1641 genomes, to facilitate a metadata driven approach to genomic analysis.

\subsection{Predictive Markers for Sub-groups}
A `group' of bacteria can be defined in numerous ways, from spatially or temporally co-located strains, to those sharing biochemical utilization patters, or those that define a clade of a phylogenetic tree. Regardless of how a group is defined, users are generally interested in defining characteristics that are predictive of the group, and can be used to discriminate it from other bacteria. SuperPhy utilizes both the presence / absence of genomic regions, as well as SNPs withing shared regions to be used to define markers statistically predictive of a group, however it may be defined. These identified biomarkers have potential downstream application in \textit{in silico} diagnostics or simple wet-lab tests for the identified markers.
...
\newpage
\begin{figure}[h!]
  \includegraphics[width=0.95\columnwidth]{images/specific_scatter.png}
  \label{fig:specific_scatter}
  \caption{The distribution among 2324 \textit{E. coli} genomes of the ten species-specific regions identified in this study, and the 3598 `conserved core' genomic regions. Genomes are plotted as dots where the size of the dot reflects genome quality, given by `genome size (Mbp)' / No. contigs.}
\end{figure}
...
\newpage
\begin{table}[h!]
\caption{The ten \textit{E. coli} species-specific genomic regions identified in this study based on a total sequence identity of 90\%, their location in the K12 reference genome MG1655, and their putative function based on the top scoring BlastX hit.}
\label{tab:specific_location_function}
      \begin{tabular}{lccl}
        \hline
        Region ID & Start bp & Stop bp & Putative function\\ 
        \hline
        3160548 & 347258  & 346259  & Propionate catabolism operon regulatory protein PrpR\\
        3160296 & 537566  & 536567  & 2-hydroxy-3-oxopropionate reductase\\
        3160113 & 538566  & 537567  & Allantoin permease\\
        3159571 & 541565  & 540567  & Purine permease ybbY\\
        3159389 & 542566  & 541567  & Glycerate kinase\\
        3158844 & 545665  & 544666  & Allantoate amidohydrolase\\
        3158667 & 546665  & 545666  & Ureidoglycolate dehydrogenase\\        
        3159808 & 1588200 & 1587201 & FimH protein\\       
        3160196 & 4411062 & 4410063 & Hypothetical protein\\
        3158082 & 4456632 & 4457631 & Mur ligase family, glutamate ligase domain protein\\
        \hline
      \end{tabular}
\end{table}
