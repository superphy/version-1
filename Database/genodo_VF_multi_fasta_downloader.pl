...
=head1 SYNOPSIS
  ...
  % genodo_VF_multi_fasta_downloader.pl [options] 
...
Script to download all Virulence Factor fasta sequences from the database to a single multi-fasta file, 
to use for generating vir/amr data and the phylogenetic tree, etc.


Sequences will have the tag:
>VF_feature_id|<uniquename>|<name>
...
sub _getVFSequenceFeatureIds {
	...
	while (my $featureIDrow = $VFfeatureIDs->next) {
		#print $featureIDrow->feature_id  . "\n";
		push(@VFIds , $featureIDrow->feature_id);
	}
	print "Retrieved public sequence id's. Writing to file...\n";
	_getVFSequences(\@VFIds);
...
sub _getVFSequences {
	my $_VFIds = shift;
	foreach my $_VFID (@{$_VFIds}) {
		my $VFSequences = $schema->resultset('Feature')->search(
			{ 'me.feature_id' => "$_VFID" },
			{
				columns => [ qw/me.feature_id me.residues me.name me.uniquename/ ]
			}
			);
		while (my $row = $VFSequences->next) {
			_writeFastaSeqToFile($row , "VF" , $_VFID);
		}
		_aggregateHeaders($_VFID , "public");
	}
}
...
sub _aggregateHeaders {
	...
	while (my $file = readdir TEMP)
	{
		...
		open my $out, '>>' , "$OUTPUTDIR/$tempFolder/$tempMultiFastaFiles/$VFTag" . "_" . "$_VFID" or croak "$!";
		while (<$in>) {
			print $out $_;
		}
	...
	closedir TEMP;
	print "File created for $VFTag" . "_" . "$_VFID\n";
}
...
sub _mergeFiles {
	...
	while (my $file = readdir TEMP2)
	{
		...
		open my $out, '>>' , "$OUTPUTDIR/VFmultiFasta$timeStamp.fasta" or croak "$!";
		while (<$in>) {
			print $out $_;
		}		
	}
	closedir TEMP2;
	print "VFmultiFasta$timeStamp.fasta created in $OUTPUTDIR\n";
}