...
sub snp_data {
	...
	if($snpo_file) {
		...
		open(my $in, '<', $snpo_file) or $logger->logdie("Error: unable to read file $snpo_file ($!)");
		while(my $line = <$in>) {
			chomp $line;

			my ($snp_id, $func) = split(/\t/, $line);
			$logger->logdie("Error: invalid format on line $line in snp order file") unless $snp_id && $func;

			push @snp_order, $snp_id;
			$snp_functions{$snp_id} = $func;
		}
